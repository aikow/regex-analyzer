...
use crate::pattern::group::GroupTree;
use crate::pattern::counter::PatternCounter;
use crate::pattern::matcher::PatternMatcher;
use crate::pattern::{Analyzer, Pattern};
...
pub fn match_file<P>(path: P, tree: Vec<GroupTree<Pattern>>, top: usize) -> Result<PatternMatcher, String>
    where P: AsRef<Path>
{
...
pub fn parse_input<P>(path: P) -> Result<Vec<GroupTree<Pattern>>, String> where P: AsRef<Path> {
    ...
    let file = File::open(path).map_err(|e| format!("{}", e))?;
    let reader = BufReader::new(file);
    let pattern_tree: HashMap<String, PatternTreeHelper> = serde_yaml::from_reader(reader).expect("Failed to parse YAML");
    ...
    fn traverse(name: String, tree: &PatternTreeHelper) -> Result<GroupTree<Pattern>, String> {
        match tree {
            ...
            PatternTreeHelper::Node(map) => {
                ...
                if !invalid.is_empty() {
                    return Err(format!("Unable to convert the following patterns: {:?}", invalid));
                }

                Ok(GroupTree::Group { name, group: patterns })
            }
        }
    }

    pattern_tree.into_iter().map(|(name, helper)| traverse(name, &helper)).collect()
}
